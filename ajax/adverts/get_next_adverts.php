...
if (!empty($_POST['page'])) {
    ...
    if (!empty($_POST['keywords'])){
        $filter[] = 'name LIKE \'%'.$adverts->mysqli->escape_string($_POST['keywords']).'%\'';
    }
    if (!empty($_POST['search'])){
        $filter[] = 'name LIKE \'%'.$adverts->mysqli->escape_string($_POST['search']).'%\'';
    }
    ...
    if (!empty($_POST['date'])){
        ...
        $date = date('Y-m-d H:i:s', strtotime('- '.$_POST['date'].' hours'));
    ...
    $pageAdvertsCount = 64;
    if (!empty($_POST['type'])) {
        if ($_POST['type'] == 'favorites') $pageAdvertsCount = 16;
        if ($_POST['type'] == 'adverts') $pageAdvertsCount = 16;
    }
    $offset = (int)$_POST['page'] * $pageAdvertsCount;

    if (empty($_POST['type'])) {
        if (!empty($filter)){
            $smarty->assign('adverts', $adverts->GetAll('status = 2 AND '.implode(' AND ', $filter), $pageAdvertsCount, $sort, $offset));
        } else{
            $smarty->assign('adverts', $adverts->GetAll('status = 2', $pageAdvertsCount, $sort, $offset));
        }
    } elseif ($_POST['type'] == 'favorites') {
        $smarty->assign('adverts', $adverts->GetAll('status = 2 AND id IN ('.implode(', ', $user->getFavorites()).')', $pageAdvertsCount, $sort, $offset));
    } elseif ($_POST['type'] == 'adverts') {
        $userId = $adverts->mysqli->escape_string($_POST['user_id']);
        $smarty->assign('adverts', $adverts->GetAll("status = 2 AND author = '$userId'", $pageAdvertsCount, $sort, $offset));
    }
    ...
    $smarty->display('elements/advert/ajax_adverts.tpl');
